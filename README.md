...
## ISSUES FOLLOWING THE LAB

The following issues came up during setting up the lab. 
- Hardware: Macbook M2

### 6.1.6

The following command doesn't work in the root folder:

```bash
docker build -t cicd-image .
```

The Dockerimage is inside the `.docker` folder. Docker doesn't look for Dockerfiles automatically in a `.docker` folder. 

Instead run:

```bash
docker build -f .docker/Dockerfile  -t cicd-image .
```
